...
export class DownloadLimitError extends Error {
    windowName;
    retryAfterMs;
    constructor(windowName, retryAfterMs) {
...
export class DownloadThrottle {
    ...
    windows;
    nowFn;
    constructor(nowFn = () => Date.now(), limits) {
        this.nowFn = nowFn;
        this.windows = {
    ...
    acquire() {
        ...
        if (retryAfter > 0) {
            const over = Object.keys(this.windows).find((n) => this.windows[n].max > 0 && this.recentWithin(n, now, this.windows[n].windowMs).length >= this.windows[n].max);
            throw new DownloadLimitError(over || 'window', retryAfter);
        }
        // 通过：记录本次
        for (const name of Object.keys(this.windows)) {
            if (this.windows[name].max <= 0)
                continue;
    ...
    release() {
        for (const name of Object.keys(this.windows)) {
            if (this.windows[name].max <= 0)
                continue;
            const stamps = this.timestamps[name];
            if (stamps && stamps.length)
                stamps.pop();
        }
        logDebug(`DownloadThrottle: released failed download (${this.usageSummary(this.nowFn())})`);
    }
    /**
    ...
    check() {
        ...
        for (const [name, cfg] of Object.entries(this.windows)) {
            ...
            if (recent.length >= cfg.max) {
                ...
                if (restore > now)
                    retryAfter = Math.max(retryAfter, restore - now);
            }
        }
        return retryAfter;
    }
    ...
    recentWithin(name, now, windowMs) {
...
//# sourceMappingURL=DownloadThrottle.js.map