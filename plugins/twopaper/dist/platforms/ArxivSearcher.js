...
export class ArxivSearcher extends PaperSource {
    ...
    async search(query, options = {}) {
        ...
        if (!validation.valid) {
            this.errorHandler.handleError(Object.assign(new Error(validation.error || 'Invalid query'), { invalidQuery: true }), 'search');
        }
    ...
    async downloadPdf(paperId, options = {}) {
        try {
            ...
            if (response.status < 200 || response.status >= 300) {
                ...
                const httpError = new Error(`arXiv download returned HTTP ${response.status}`);
