        ...
        try {
            ...
            while (papers.length < maxResults) {
                ...
                const response = useOpenAccess
                    ? await ErrorHandler.retryWithBackoff(() => this.openAccessClient.get('/json', { params }), { context: 'Springer OpenAccess search' })
                    : await ErrorHandler.retryWithBackoff(() => this.metadataClient.get('/json', { params }), { context: 'Springer Meta search' });
                ...
                else if (Array.isArray(response.data.result) && response.data.result[0]?.title) {
