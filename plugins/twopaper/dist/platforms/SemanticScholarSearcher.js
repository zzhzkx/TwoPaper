...
export class SemanticScholarSearcher extends PaperSource {
    ...
    constructor(apiKey) {
        ...
        this.detailsCache = new RequestCache({
            maxSize: 100,
            ttlMs: 3600000 // 1 hour
        });
    }
     ...
     */
    async requestWithRetryBackoff(fn, context) {
        const maxRetries = 3;
        let lastError;
    ...
    async search(query, options = {}) {
        ...
        try {
            while (collected.length < maxResults) {
                ...
                const response = await this.requestWithRetryBackoff(() => axios.get(url, { params, headers, timeout: TIMEOUTS.DEFAULT, maxRedirects: 5 }), 'Semantic Scholar search');
    ...
    async getPaperDetails(paperId) {
        ...
        try {
            ...
            const response = await this.requestWithRetryBackoff(() => axios.get(url, { params, headers, timeout: TIMEOUTS.DEFAULT, maxRedirects: 5 }), 'Semantic Scholar paper details');
    ...
    async downloadPdf(paperId, options = {}) {
        try {
            ...
            await this.rateLimiter.waitForPermission();
            const response = await this.requestWithRetryBackoff(() => axios.get(paper.pdfUrl, {
                responseType: 'stream',
 ...
 */
function sanitizePaperId(input) {
