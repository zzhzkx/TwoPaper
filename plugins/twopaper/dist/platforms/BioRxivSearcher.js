...
export class BioRxivSearcher extends PaperSource {
    ...
    async search(query, options = {}) {
        ...
        if (!validation.valid) {
            this.handleHttpError(Object.assign(new Error(validation.error || 'Invalid query'), { invalidQuery: true }), 'search');
        }
        // 钳到统一上限,避免无限拉取
        ...
        try {
            ...
            for (let cursor = 0; papers.length < maxResults && (total === undefined || cursor < total); cursor += this.pageSize) {
                ...
                if (!message || message.status !== 'ok') {
                    ...
                    this.handleHttpError(Object.assign(new Error(`${this.serverType} API returned non-ok state: ${msg}`), {
                        status: 200,
     ...
     */
    async searchByKeywordScan(query, options) {
        const validation = validateQueryComplexity(query, {
            maxLength: SEARCH_LIMITS.MAX_QUERY_LENGTH,
            maxBooleanOperators: SEARCH_LIMITS.MAX_BOOLEAN_OPERATORS
        });
        ...
        try {
            ...
            for (let page = 0; page < this.MAX_SCAN_PAGES; page++, cursor -= PAGE) {
                if (papers.length >= maxResults || cursor < 0 || Date.now() > deadline)
                    break;
                ...
                for (const item of collection) {
                    ...
                    if (seen.has(key))
                        continue;
                    seen.add(key);
                    const paper = this.parseSearchResponse({ collection: [item] }, query, options)[0];
                    if (paper)
                        papers.push(paper);
