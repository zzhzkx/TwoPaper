...
export function resolveProjectDir() {
    const injected = process.env.CLAUDE_PROJECT_DIR;
...
export function resolveOutputRoot() {
    const explicit = process.env.TWOPAPER_OUTPUT_DIR;
    if (explicit && explicit.trim() !== '') {
        return path.resolve(explicit);
    }
    return path.join(resolveProjectDir(), 'twopaper');
}
export default { resolveProjectDir, resolveOutputRoot };
