...
const CREDENTIAL_MAP: Omit<CredentialEntry, 'configured'>[] = [
  {
    env: 'WOS_API_KEY',
...
export const WRITABLE_ENV_KEYS: ReadonlySet<string> = new Set(CREDENTIAL_MAP.map((c) => c.env));

export function collectCredentials(): CredentialEntry[] {
...
export function missingCredentials(): CredentialEntry[] {
...
export function writeCredentials(
  ...
  for (const [key, rawValue] of Object.entries(updates)) {
    if (!WRITABLE_ENV_KEYS.has(key)) continue;
