...
export function currentBridgeMode(): BridgeMode {
...
export class BridgesClient {
  ...
  runAutoDownload(identifier: string, outputDir?: string): Promise<string> {
    return new Promise((resolve, reject) => {
      ...
      if (outputDir) args.push('--output_dir', outputDir);
      ...
      child.on('close', (code) => {
        ...
        else reject(new Error(`scansci CLI exited ${code}: ${out || 'no output'}`));
      });
    });
  }

  private cmdAvailable(cmd: string): boolean {
    const exe = cmd.split(/\s+/)[0];
    const avail = process.env.PATH?.split(/[;:]/).some((dir) => {
      try {
        const candidates =
          process.platform === 'win32' ? [`${exe}.exe`, `${exe}.cmd`] : [exe];
