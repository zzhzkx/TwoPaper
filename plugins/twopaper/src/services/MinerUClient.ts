...
export class MinerUClient {
   ...
   */
  async pdfToMarkdown(pdfPath: string): Promise<MinerUResult> {
    if (!this.token) {
    ...
    if (!uploadUrl) throw new Error('MinerU: no signed upload URL returned');
    ...
    if (!putRes.ok) throw new Error(`MinerU upload failed: HTTP ${putRes.status}`);
  ...
  private async requestBatch(name: string): Promise<{ batchId: string; uploadUrl: string }> {
  ...
  private async pollBatch(batchId: string, name: string): Promise<string> {
    ...
    while (Date.now() < deadline) {
      ...
      if (res.ok) {
        const data = await res.json();
        const item = (data?.data?.extract_result || []).find((r: any) => r.file_name === name);
        if (item) {
  ...
  private async downloadZip(zipUrl: string): Promise<AdmZip> {
   ...
   */
  private extractFromZip(zip: AdmZip, _name: string): { markdown: string; images: { rel: string; data: Buffer }[] } {
    ...
    if (!mdEntry) throw new Error('MinerU zip did not contain a markdown file');
    ...
    const images = entries
      ...
      .map((e) => {
        ...
        const base = e.entryName.split(/[\\/]/).pop() as string;
  ...
  private raiseHttp(res: Response, op: string): void {
    if (!res.ok) {
