...
export interface PlatformEntry {
  platform: string;
  /** 搜索/下载/全文能力 */
  ability: { search: boolean; download: boolean; fulltext: boolean };
  /** 是否需要 API key */
...
export class PlatformRegistry {
  ...
  async getStatus(searchers: Searchers, validate = false): Promise<PlatformRow[]> {
  ...
  private lookupSearcher(searchers: Searchers, platform: string): any {
    if (platform === 'oa') return undefined; // OA 源是服务，不是 searcher
    const key = platform as keyof Searchers;
