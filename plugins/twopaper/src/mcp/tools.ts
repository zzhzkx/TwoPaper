...
export const TOOLS: Tool[] = [
  ...
  },
  {
    name: 'get_oa_pdf',
    description:
      "Locate a legal open-access PDF for a paper (Unpaywall/OpenAlex/Europe PMC) by DOI or title. Returns the PDF URL and license when found. Requires OA_EMAIL for Unpaywall/Europe PMC; OpenAlex optional OPENALEX_API_KEY.",
    inputSchema: {
      type: 'object',
      properties: {
  ...
  },
  {
    name: 'get_fulltext',
    description:
      "Get full-text Markdown of a paper via MinerU (PDF → Markdown). Accepts a DOI/paperId (auto-download) or an existing PDF path. Requires MINERU_TOKEN. Returns markdown text + cached .md path for host-side analysis.",
    inputSchema: {
      type: 'object',
      properties: {
        doi: { type: 'string', description: 'DOI of the paper' },
  ...
  },
  {
    ...
    description:
      'Read-only check of the scansci download bridge: mode (none/hint/auto), whether configured, and whether the scansci CLI is reachable.',
    inputSchema: {
      type: 'object',
      properties: {}
    }
