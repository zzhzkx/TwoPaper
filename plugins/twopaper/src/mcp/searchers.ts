...
export function selectSearchable(
  registry: Searchers,
  opts: { includeScholar?: boolean; exclude?: readonly string[] } = {}
): Array<[string, Searchers[keyof Searchers]]> {
  const extra = new Set(opts.exclude || []);
  const seenInstances = new Set<unknown>();
  const picked: Array<[string, Searchers[keyof Searchers]]> = [];

  for (const [key, searcher] of Object.entries(registry)) {
    if (ALIAS_KEYS.has(key) || extra.has(key)) continue;
    // 同一实例只取第一个真实 key（别名已在上一步过滤）
    if (seenInstances.has(searcher)) continue;
    seenInstances.add(searcher);
    if (key === 'googlescholar' && !opts.includeScholar) continue;
    picked.push([key, searcher as Searchers[keyof Searchers]]);
  }
  return picked;
}
